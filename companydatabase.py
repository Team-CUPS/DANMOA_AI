...
# 크롤링된 데이터를 Firestore에 저장하는 함수
def save_job_to_firestore(job_data):
    doc_ref = db.collection(u'jobs').document()  # Firestore에 고유 ID로 새 문서를 생성
    doc_ref.set(job_data)

# 크롤링 함수 (이미 정의된 함수를 사용)
...
for offset in range(0, total_data_count, limit):
    ...
    if parse_company and 'data' in parse_company and parse_company['data']:
        ...
        for recur_id in recur_ids:
            ...
            info_keys = ['COMPANY','TITLE', 'MAINDUTIES', 'QUALIFICATION', 'PREFERENTIAL',
                          'JOB_POSTING_URL','ID']  # 필요한 정보 키
...
for job in all_data:
    # 필요한 필드만 선택합니다.
    job_to_save = {k: job[k] for k in ['QUALIFICATION', 'MAINDUTIES', 'COMPANY', 'PREFERENTIAL'] if k in job}
    
    # Firestore에 데이터를 저장합니다.
    save_job_to_firestore(job_to_save)

print("All job data uploaded to Firestore.")
